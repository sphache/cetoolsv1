...
#'
#' @param p a ggplot
#' @param filename size of the legend symptoms
#' @param height size of text
#' @param width space between legend elements
#' @param font_family font
#' @examples
#' export_plotly2SVG(p, filename = "violin.sensitive", parent_path = getwd(),
#  height = 300, width = 850, font_family = "Arial")
#  closeSeleniumDriver()
...
export_plotly2SVG <- function(plotly_graph,
                              ...
                              width = 800, # previously 800
                              height = 600, # previously 600
                              remove_title = FALSE,
                              font_family = "Arial",
                              incl_PDF_copy = FALSE,
                              incl_PNG_copy = FALSE,
                              ...
                              autocrop_png = TRUE)
{
  ...
  fprof <- RSelenium::makeFirefoxProfile(list(browser.download.dir = parent_path,
                                              browser.download.folderList = 2L,
                                              browser.download.manager.showWhenStarting = FALSE,
                                              browser.download.manager.useWindow = FALSE)
  )

  selenium_driver <- RSelenium::rsDriver(browser = "firefox", version = "3.141.59", geckover = "0.26.0", verbose = FALSE, check = TRUE, extraCapabilities = fprof)
  ...
  if ( incl_PDF_copy )
  {
    rsvg::rsvg_pdf(svg = filepath,
                   file = paste0(tools::file_path_sans_ext(filepath), ".pdf"))
  }
